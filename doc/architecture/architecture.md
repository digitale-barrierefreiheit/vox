...
- **A CI performance-regression gate** on stable, dedicated hardware compares against a baseline and fails on regressions beyond a relative threshold (use relative thresholds to absorb CI noise). Realised as the `benchmarks` job: pipeline percentiles are compared against the `dev` baseline (the `benchmark-data` branch) at a generous 200% threshold — hosted runners are noisy and not yet the dedicated hardware this section assumes, so the absolute in-process budget above stays the authoritative gate.
