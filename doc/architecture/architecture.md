...
- The **SonarCloud new-code coverage gate therefore measures everything.** No production source is excluded from coverage; only the tests are out — classified as the test partition (`sonar.test.inclusions`) rather than measuring their own coverage. The handful of irreducibly OS-bound lines (the live-modifier read, the message loop, the callback's event unpacking) are exercised by the labelled `*_itest` integration tests.
