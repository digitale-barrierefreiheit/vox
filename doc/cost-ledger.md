...
- **SonarCloud LOC (`ncloc`):** 13,862 as read on 2026-06-18 — **0 paid LOC** (this is a public project; SonarCloud is free for public projects).
...
- **GitHub Actions minutes** (window 2026-06, GitHub-hosted runners, imputed at the 2026-01-01 list rates):
  ...
  |-----------|--------:|-----:|---------------:|
  | Linux | 4,763 | 2,668 | $28.58 |
  | Windows | 8,324 | 762 | $83.24 |
  | **Total** | | | **$111.82** |
...
|------|----------|------------------|-----------|
| Org Actions billing cross-check (factor 3) | **v1:** the `cost-collector` workflow reads `COST_BILLING_TOKEN` (a **fine-grained PAT**, org **Administration: read**, resource owner = the org) **if that secret is set** — until an org owner provisions it, this line shows *"not configured (prerequisite)"*. **Recommended hardening:** an org-owned GitHub App, installation token minted per run via `actions/create-github-app-token`, to avoid a long-lived PAT. | Org owner: create a fine-grained PAT with Administration: read and store it as `COST_BILLING_TOKEN`; *(hardening)* later create + install the App and switch the workflow to mint its token. | `COST_BILLING_TOKEN` *(hardening adds `COST_APP_ID`, `COST_APP_PRIVATE_KEY`)* |
