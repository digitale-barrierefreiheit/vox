...
|------|----------|------------------|-----------|
| Org Actions billing cross-check (factor 3) | **v1 (wired):** the `cost-collector` workflow reads a **fine-grained PAT** (org **Administration: read**, resource owner = the org) from `COST_BILLING_TOKEN`. **Recommended hardening (not yet wired):** an org-owned GitHub App, installation token minted per run via `actions/create-github-app-token`, to avoid a long-lived PAT. | Org owner: create a fine-grained PAT with Administration: read and store it as `COST_BILLING_TOKEN`; *(hardening)* later create + install the App and switch the workflow to mint its token. | `COST_BILLING_TOKEN` *(hardening adds `COST_APP_ID`, `COST_APP_PRIVATE_KEY`)* |
