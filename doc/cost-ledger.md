...
| 9 | AI code review, e.g. GitHub Copilot (**maintainer-contributed**) | `ai-review.json`, updated out-of-band via a `repository_dispatch` (type `ai-review-cost`) sent from the maintainer's separate billing context | **none in this repo** (no third-party billing credentials are stored here) | per-month, project-level | **Yes** (reported in) | GitHub exposes **no per-repository Copilot attribution**; the figure is the maintainer's good-faith monthly disclosure of the AI-review cost attributable to Vox. The billing entity and mechanism are intentionally kept out of this public repo. |
